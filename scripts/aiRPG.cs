...
{
	echo("AIConnection::onAIConnect(" @ %client @ ", " @ %name @ ", " @ %team @ ", " @ %skill @ ", " @ %offense @ ", " @ %voice @ ", " @ %voicePitch @ ")");
...
{
	echo("AIConnection::startMission(" @ %client @ ")");
...
{
	echo("AIConnection::onAIDrop(" @ %client @ ")");
...
//check if using magic as a main way of attacking
	if(%targetId !$= -1)
	{
		...
		if(!fetchdata(%client, "magic"))
		{
            ...
            if(%haslos) {
                if(fetchdata(%client, "allowedspells") !$= "") {
                    for(%i = 0; %i < 3; %i++) {
                       %d = getRandom(0, getWordCount(fetchdata(%client, "allowedspells")));
                       %spell = getWord(fetchdata(%client, "allowedspells"), %d);
...
function AIRPGFishWanderTask::weight(%task, %client)
{
	...
	{
		echo("ERROR: Task should not have been called - aiRPG.cs AIRPGFishWanderTask Client:" SPC %client SPC " Playerid:" SPC %client.player SPC "Task:" SPC %task);
		//%task.delete();
		return;
	}
	if(%client.getEngageTarget() !$= -1)
