...
function execrpg()
{
    ...
    exec("scripts/rpgboss.cs");
    exec("scripts/rpgspecialatt.cs");
...
function RPGGame::startMatch(%game)
{
	...
	%game.recursiveWorld(1); // start the world recursive
	%game.switchToDay();
}
function RPGGame::recursiveWorld(%game, %ticks)
{
	if($debugMode == true) echo("RPGGame::recursiveWorld(" @ %game @ "," SPC  %ticks @ ");");
	if(iseventpending(%game.recursive)) return;
	if($displaydebugticks) echo("tick:" @ %ticks);
	if($dayNightCycle < 60 && $daynightcycle != -1) $dayNightCycle = 4 * 60 * 60;//set to default if too low
	

	if($saveworlddelay != 0)
	if(%ticks % $saveworlddelay == 0)
	{
		SaveWorld();
	}
	if($daynightcycle != 0) //bleh UE UE
	if( (%ticks % $daynightcycle == 0) && $daynightcycle != -1)
	{
	
		if(%game.daynight == 1  )
		{
			echo("switching from day to night" SPC %ticks);
			%game.daynight = 0;
			game.switchtodusk();
			%game.switchsky = %game.schedule($daynightcycle/10*1000, "switchtonight"); 
		}
		else
		{
			echo("switching from night to day" SPC %ticks);
			%game.daynight = 1;
			game.switchtodusk();
			%game.switchsky = %game.schedule($daynightcycle/10*1000, "switchtoday"); 
		}
	}
	...
	if(%ticks > mpow(2,31))
	%ticks = 0;
	%game.recursive = %game.schedule( 1000, "recursiveWorld", %ticks+1);
}
function RPGGame::exportDay(%game)
{
	if($debugMode == true) echo("RPGGame::exportDay(" @ %game @ ");");
	%obj = rpgdayeffect;
 	%obj.save("temp/day.cs");
...
function RPGGame::onClientDamaged(%game, %clVictim, %clAttacker, %damageType, %sourceObject, %weapon, %pos, %damloc)
{
	if($debugMode == true) echo("RPGGame::onClientDamaged(" @ %game @ "," SPC %clVictim @ "," SPC %clAttacker @ "," SPC %damageType @ "," SPC %sourceObject @ "," SPC %weapon @ "," SPC %pos @ "," SPC %damloc @ ");");
	...
	if(%clVictim > 0)
	{
        ...
        else if(%damageType == $DamgeType::SpecialAttack) {
           %clVictim.DoSpecialAttackDamage(%clAttacker, %clAttacker.lastSpecial);
           return; //Special Attacks Handle Damage Differently
        }
		else if(%damageType != ($DamageType::Ground || $DamageType::Impact) && %damagetype != $DamageType::Spell && %damageType != $DamageType::SpecialAttack)
		{
		...
		if(%damageType == $DamageType::Lava && %clattacker == 0)
		{
			%value = fetchdata(%clvictim, "HP");
			%value = (%value / $TribesDamageToNumericDamage);
		}
		//-------------------------------------------------
		...
		//-------------------------------------------------
		if(%clVictim.adminLevel >= 4 && %damageType $= ($damageType::Ground || $damageType::Impact) && fetchdata(%clVictim, "godMode") == 1)
			%value = 0;
		if(%clVictim.adminLevel >= 5 && fetchdata(%clVictim, "godMode") == 1)
			%value = 0;
		...
		{
//			if(Zone::getType(fetchData(%clVictim, "zone")) $= "PROTECTED" && Zone::getType(fetchData(%clAttacker, "zone")) !$= "PROTECTED")
//			{
...
//				%sameRaceNull = true;
//			}
//			else if(Zone::getType(fetchData(%clVictim, "zone")) !$= "PROTECTED" && Zone::getType(fetchData(%clAttacker, "zone")) !$= "PROTECTED")
//			{
//				//echo("both guys outside, do damage");
//			}
//			else if(Zone::getType(fetchData(%clVictim, "zone")) $= "PROTECTED" && Zone::getType(fetchData(%clAttacker, "zone")) $= "PROTECTED" && %clAttacker !$= %clVictim)
//			{
...
//			}
			...
			//echo(IsInTargetList(%clattacker, %clvictim) SPC IsInTargetList(%clvictim, %clattacker));
		...
		if(!IsDead(%clVictim))
		{
			...
			if(%value)
			{
				...
				if(%convValue > 0)
				{
					if(%clAttacker $= %clVictim || %damagetype == $DamageType::Ground )
					{
							%hitby = "yourself";
					}
					...
					if(%damageType == $DamageType::Lava )
					{
						radiusAllExcept(%clVictim, 0, $MsgBeige @ %dname @ " wasn't watching where he was going. (lava death)");
					}
					else
...
function Armor::damageObject(%data, %targetObject, %sourceObject, %position, %amount, %damageType, %momVec)
{
	...
	if(%clVictim $= "")
	{
		//check to see if this is a townbot, if so break out!
		%townbot = false;
		...
		if(%townbot) return false;
	}
 
    //P139 Special Attack Overide
    if(%damageType == $DamageType::SpecialAttack) {
	   %clAttacker = %sourceObject.sourceObject;
	   Game.onClientDamaged(%clVictim, %clAttacker, %damageType, %sourceObject, fetchData(%clVictim, "weaponInHand"), %position,  %targetObject.getDamageLocation(%position));
...
function RPGGame::onClientLeaveGame(%game, %client)
{
	...
	echo("RPGGAME::onClientLeaveGame(" @ %game @ ", " @ %client @ ")");
...
{
	echo("GameConnection::onConnect(" @ %client @ ", " @ %name @ ", " @ %raceGender @ ", " @ %skin @ ", " @ %voice @ ", " @ %voicePitch @ ")");
...
echo("Client Version:" SPC %ver);
	...
	else
	{
		...
		if(!$isdeveloperversion)
	 	%client.setDisconnectReason( "Incompatable version! The servers version is "@$rpgver@". Go to www.phantomdev.net/Tribes2/ISRPG/ to obtain the correct version." );
	        else
	        %client.setDisconnectReason( "Incompatable version! This server is running a developer version. Please connect to a different server to play T2rpg. T2rpg can be downloaded at www.phantomdev.net/Tribes2/ISRPG/.");
	        //%client.drop();
	        %client.schedule(200, "delete");
		//invalid version KICK!
		return true;
	}
}
...
{
	%client.rpgver = %ver;
	echo("Got Client Version");
}
function GameConnection::onDrop(%client, %reason)
{
	echo("GameConnection::onDrop(" @ %client @ ", " @ %reason @ ")");
	...
	if($missionRunning)
	{
		if(inArena(%client))
		{
			...
			{
				Player::scriptKill(%client.player);//end match
			}
	...
	else if(%check == %client.selclient && %client.selclient !$= "")
	{
		if(%where == 0)
		{
			...
			else
			{
				...
				else if(!(IsInWhichParty(%client.selclient) != false) && fetchData(%client, "partyOwned"))
				commandToclient(%client, 'ISMenuAddRow', 5, "5: Invite To Party");
				
				//if(GetWord($rpggame::parties[fetchdata(%client, "Party")], 0) == %client)
				//commandToClient(%client, 'ISMenuAddRow', 5, "5: " @ (IsInPartyList(%client, %client.selclient) ? "Kick from Party" : "Invite to join party"));
			}
		}
