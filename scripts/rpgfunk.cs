...
function NONE::SaveCharacter(%game, %client, %status)
{
	return;//creating new system do not allow saving.
	if(%client.isaicontrolled()) return;
	if(fetchdata(%client, "CLASS") $= "")return;//doh, this is to make sure messed up files do not get deleted.
	%file = new FileObject();
	%didsave = 0;
	if(!%client.guid)
	%client.guid = 0;
	%filename = "characters/" @ %client.guid @ "/" @ FileFriendly(%client.namebase) @ ".cs";//lets try this instead, use guid instaed
	if(%status $= "FAILED")
	 %filename = "characters/ERRORS/" @ %client.guid @ "/" @ FileFriendly(%client.namebase) @ ".cs";//document the error 
	//if the file exists delete it... then store the new one... 
	//i wonder if we can just export some vars...
	if(!isWriteableFileName(%filename))
	return 0;// file is readonly, cant write to it drop here.
	echo("Saving character: " @ %client.rpgname @ ".");
	if(%file.openForWrite(%filename))
	{
		%newname = RPGFixString(%client.namebase);
		echo("creating save file for " @ %client.namebase @" (" @ %client @ ".");
		//we actually make direct calls in the persons loadcharacter so that way in future versions we wont have to 'translate'
		%file.writeline("//" @ %client.namebase @"");
		%file.writeline("function LoadCharacter::d" @ %newname @ "(%client) {");//to prevent doubleing
		
		%file.writeline("storedata(%client, \"EXP\", " @ fetchdata(%client, "EXP")+1  @ "-1);");
		%file.writeline("storeData(%client, \"RACE\", \""@ fetchdata(%client, "RACE") @"\");");
		%file.writeline("storedata(%client, \"COINS\", " @ fetchdata(%client, "COINS")+1  @ "-1);");
		%file.writeline("storeData(%client, \"GROUP\", \""@ fetchdata(%client, "GROUP") @"\");");
		%file.writeline("storeData(%client, \"CLASS\", \""@ fetchdata(%client, "CLASS") @"\");");
		%file.writeline("storeData(%client, \"SP\", "@ fetchdata(%client, "SP")+1 @"-1);");
		if(%client.player == 0)
		if (%client.lastpos != 0)
		%file.writeline("storeData(%client, \"campPos\", \""@ %client.lastpos @"\");");
		else
		%file.writeline("storeData(%client, \"campPos\", \"\");");
		else
		%file.writeline("storeData(%client, \"campPos\", \""@ %client.player.getTransform() @"\");");
		%file.writeline("storeData(%client, \"BANK\", " @ fetchdata(%client, "BANK") @");");
		%file.writeline("storeData(%client, \"grouplist\", \"" @ fetchdata(%client, "grouplist") @ "\");");
		%file.writeline("storeData(%client, \"defaultTalk\", \""@ syntaxstring(fetchdata(%client, "defaulttalk")) @"\");");
		%file.writeline("storeData(%client, \"LCK\", \"" @ fetchdata(%client, "LCK")@ "\");");
		%file.writeline("storeData(%client, \"PlayerInfo\", \""@ syntaxstring(fetchdata(%client, "playerinfo")) @"\");");
		%file.writeline("storeData(%client, \"ignoreGlobal\", \""@ fetchdata(%client, "ignoreglobal") @"\");");
		%file.writeline("storeData(%client, \"LCKconsequence\", \""@ fetchdata(%client, "LCKconsequence") @"\");");
		%file.writeline("storeData(%client, \"tmphp\", \"" @ fetchdata(%client, "HP") @ "\");");
		%file.writeline("storeData(%client, \"tmpmana\", \"" @ fetchdata(%client, "MANA") @ "\");");
		%file.writeline("storeData(%client, \"tmpname\", \"" @ fetchdata(%client, "tmpname") @ "\");");
		%file.writeline("storedata(%client, \"QuickBind0\", \""  @ syntaxstring(fetchdata(%client, "QuickBind0")) @"\");");
		%file.writeline("storedata(%client, \"QuickBind1\", \""  @ syntaxstring(fetchdata(%client, "QuickBind1")) @"\");");
		%file.writeline("storedata(%client, \"QuickBind2\", \""  @ syntaxstring(fetchdata(%client, "QuickBind2")) @"\");");
		%file.writeline("storedata(%client, \"QuickBind3\", \""  @ syntaxstring(fetchdata(%client, "QuickBind3")) @"\");");
		%file.writeline("storedata(%client, \"QuickBind4\", \""  @ syntaxstring(fetchdata(%client, "QuickBind4")) @"\");");
		%file.writeline("storedata(%client, \"QuickBind5\", \"" @ syntaxstring(fetchdata(%client, "QuickBind5")) @"\");");
		%file.writeline("storedata(%client, \"TagSkill1\", " @ fetchdata(%client, "TagSkill1")+1-1 @ ");");
		%file.writeline("storedata(%client, \"TagSkill2\", " @ fetchdata(%client, "TagSkill2")+1-1 @ ");");
		%file.writeline("storedata(%client, \"TagSkill3\", " @ fetchdata(%client, "TagSkill3")+1-1 @ ");");
		%file.writeline("storeData(%client, \"tmpLastSaveVer\", " @ $rpgver @ ");");
		%file.writeline("storeData(%client, \"bounty\", "@ fetchdata(%client, "bounty") @");");
		%file.writeline("storeData(%client, \"isMimic\", \""@ fetchdata(%client, "isMimic") @"\");");
		%file.writeline("storeData(%client, \"MyHouse\", \""@ fetchdata(%client, "myhouse") @"\");");
		%file.writeline("storeData(%client, \"RankPoints\", "@ fetchdata(%client, "rankpoints") @");");
		%file.writeline("storeData(%client, \"ZoneMusic\", \"" @ fetchdata(%client, "zone").MusicType @ "\");"); 
		if(Game.IsJailed(%client))
			%file.writeline("Game.jail(%client, " @ Game.GetJailTime(%client) @ "*1000, " @ Game.GetJailNumber(%client) @ ");");
		for(%i = 1; $SkillDesc[%i] !$= ""; %i++)
		{
			%skill = GetPlayerSkill(%client, %i);
			%file.writeline("%client.PlayerSkill[" @ %i @ "] = " @ %skill @ ";");
			%file.writeline("$SkillCounter[%client, "@ %i @"] = " @ $SkillCounter[%client, %skilltype]+1-1 @ ";");
		}
		%inv = fetchData(%client, "inventory");
		if(inArena(%client))
		%inv = fetchData(%client, "tmpInv");
		for(%i = 0; (%itemId = GetWord(%inv, %i)) !$= ""; %i++)
		{
			if(%itemid != 0 && $InvInfo[%itemId, item] !$= "")
			%file.writeline("AddToInventory(%client, GenerateUniqueID(), \"" @ $InvInfo[%itemId, item] @ "\", " @  $InvInfo[%itemid, prefix] @ ", " @ $InvInfo[%itemid, Suffix] @ ", " @ $InvInfo[%itemId, equipped] @ ");");
		}
		%storage = fetchData(%client, "storage");
		for(%i = 0; (%itemId = GetWord(%storage, %i)) !$= ""; %i++)
		{
			if(%itemid != 0)
			%file.writeline("AddToStorage(%client, CreateItem(\"" @ $InvInfo[%itemId, item] @ "\", " @  $InvInfo[%itemid, prefix] @ ", " @ $InvInfo[%itemid, Suffix] @ "));");
		}
		for(%i = 0; %i < QuestGroup.getCount(); %i++)
		{
			%obj = Questgroup.getObject(%i);
			%obj.saveclient(%client, %file);
		}
		%file.writeline("}");
		%file.close();
		deletefile(%filename @ ".dso");// KILL the current dso
		%didsave = compile(%filename);//make the new dso
		if(!isfile(%filename @ ".dso") && %status !$= "FAILED") savecharacter(%client, "FAILED");
		if($compilesaves)
		{
		//delete the .cs! neg on hax!
		//deletefile(%filename);//bye bye cs version
		}
		%file.delete();//delete the file object
	}
	else
		return 0; //savecharacter failed!!
	return %didsave;
}
//<--- *marker
...
function RPGGame::GiveDefaults(%game, %client)
{
	...
	AddToInventory(%client, 1, "PickAxe", 3, 1, false);
	AddToInventory(%client, 3, "BluePotion", 3, 1, false);
    AddToInventory(%client, 1, "Club", 3, 1, false);
	AddToInventory(%client, 1, "Hatchet", 3, 1, false);
